...
public:
    ...
    StepResult step_flat(const std::vector<double>& flat_actions) {
        std::vector<Action> actions(drones_.size());

        for (size_t i = 0; i < drones_.size(); ++i) {
            actions[i].ax = flat_actions[2 * i];
            actions[i].ay = flat_actions[2 * i + 1];
        }
    ...
    size_t action_size() const {
        return drones_.size() * 2;
    }

    size_t observation_size() const {
        return drones_.size() * 6;
    }
